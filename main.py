...
MODULE_DATA: dict = {
    ...
    "welcome":      {"title": "👋  WELCOME SYSTEM",    "color": 0x57F287, "gif": None, "description": "Send custom welcome and goodbye messages when members join or leave.", "fields": [("Welcome Commands", "`/welcome setchannel` — Set the welcome channel\n`/welcome setmessage` — Set custom message (use {user} {server} {count})\n`/welcome disable` — Disable welcome/goodbye messages\n`/welcome test` — Preview your welcome message"), ("Goodbye Commands", "`/goodbye setchannel` — Set a dedicated goodbye channel\n`/goodbye setmessage` — Set goodbye message (use {user} {server} {username})\n`/goodbye test` — Preview your goodbye message")]},
...
class ModuleSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="Join to Create",     emoji="🔊", value="jtc",           description="JTC — auto temp voice channels"),
...
class Moderation(commands.Cog):
    ...
    async def help_command(self, interaction: discord.Interaction):
        ...
        # Row 6 — Voice
        embed.add_field(name="🔊  Join to Create", value="JTC — auto\ntemp voice channels",      inline=True)
...
class Welcome(commands.Cog):
    ...
    def _cfg(self, guild_id: int) -> dict:
        ...
        if key not in self.data:
            self.data[key] = {"channel_id": None, "goodbye_channel_id": None, "message": "Welcome {user} to **{server}**! You are member #{count} 🎉", "goodbye": "**{user}** has left {server}. Goodbye! 👋", "enabled": False}
        if "goodbye_channel_id" not in self.data[key]:
            self.data[key]["goodbye_channel_id"] = None
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if not cfg.get("enabled"):
            return
        goodbye_ch_id = cfg.get("goodbye_channel_id") or cfg.get("channel_id")
        if not goodbye_ch_id:
            return
        ch = member.guild.get_channel(goodbye_ch_id)
        if ch is None:
            return
    ...
    goodbye_group = app_commands.Group(name="goodbye", description="Configure goodbye channel", default_permissions=discord.Permissions(administrator=True))

    @goodbye_group.command(name="setchannel", description="Set a dedicated goodbye channel (separate from the welcome channel)")
    @app_commands.describe(channel="The channel to send goodbye messages in")
    async def goodbye_setchannel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        cfg = self._cfg(interaction.guild.id)
    ...
    @goodbye_group.command(name="setmessage", description="Set the goodbye message (use {user} {server} {username})")
    @app_commands.describe(message="The goodbye message template")
    async def goodbye_setmessage(self, interaction: discord.Interaction, message: str):
        cfg = self._cfg(interaction.guild.id)
        cfg["goodbye"] = message
        self._save()
        preview = message.replace("{user}", str(interaction.user)).replace("{server}", interaction.guild.name).replace("{username}", str(interaction.user))
        await interaction.response.send_message(f"✅ Goodbye message updated.\n**Preview:** {preview}", ephemeral=True)

    @goodbye_group.command(name="test", description="Send a test goodbye message to the goodbye channel")
    async def goodbye_test(self, interaction: discord.Interaction):
        cfg = self._cfg(interaction.guild.id)
        goodbye_ch_id = cfg.get("goodbye_channel_id") or cfg.get("channel_id")
        if not goodbye_ch_id:
            await interaction.response.send_message("❌ No goodbye channel set. Use `/goodbye setchannel` first.", ephemeral=True)
            return
        ch = interaction.guild.get_channel(goodbye_ch_id)
        if ch is None:
            await interaction.response.send_message("❌ Goodbye channel not found.", ephemeral=True)
            return
        msg = cfg["goodbye"].replace("{user}", str(interaction.user)).replace("{server}", interaction.guild.name).replace("{username}", str(interaction.user))
        embed = discord.Embed(description=msg, color=discord.Color.red(), timestamp=datetime.datetime.now(datetime.timezone.utc))
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
        embed.set_footer(text="This is a test message")
        await ch.send(embed=embed)
        await interaction.response.send_message(f"✅ Test goodbye message sent to {ch.mention}.", ephemeral=True)
...
class JoinToCreate(commands.Cog):
    """
    Full Join-to-Create feature set.
